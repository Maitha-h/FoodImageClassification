    ...
    for category in Catagories:
        ...
        for img in os.listdir(path):
            try:
                ...
                img_array = cv2.cvtColor(img_array, cv2.COLOR_BGR2RGB)
                new_array = cv2.resize(img_array, (IMG_SIZE, IMG_SIZE))
                random_num = random.randint(-45, 45)
                img2 = scipy.ndimage.rotate(new_array, random_num, mode='nearest',reshape=False)
                # img2 = cv2.resize(img2, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_CUBIC)
                # img2 = scipy.ndimage.zoom(img2, 2, mode='nearest')
...
def create_dataset_augmentation_reflection():
    i = 0
    j = 0
    for category in Catagories:
        path = os.path.join(DATADIR, category)
...
def separate_data():
    ...
    for i in range(len(Dataset)):
        if (i * 3) % 10 % 4 is 0:  # save 30% of data as testing data.
...
create_dataset()
#create_dataset_augmentation_reflection()
